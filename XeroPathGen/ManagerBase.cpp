...
bool ManagerBase::processPath(const QString& path)
{
	QStringList filters;
...
bool ManagerBase::getJSONStringValue(QFile &file, QJsonDocument& doc, const char* name, QString& value)
{
...
bool ManagerBase::getJSONStringValue(QFile& file, QJsonObject& obj, const char* name, QString& value)
{
...
void ManagerBase::copyDefaults(const QString& subdir)
{
	if (default_dir_.length() == 0)
		return;

	QString exedir = QCoreApplication::applicationDirPath();
	QString src = exedir + "/" + QString(subdir);

	QDir srcdir(src);
	QDir destdir(default_dir_);

	if (!destdir.exists())
	{
		if (!destdir.mkpath(default_dir_))
			return;
	}
