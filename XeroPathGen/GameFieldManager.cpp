...
{
	QString path = getDefaultDir() + "/" + filename ;
	return QFile::exists(path);
}

void GameFieldManager::convert(const QString &units)
{
	for (auto field : fields_)
		field->convert(units);
}

bool GameFieldManager::exists(const QString& name)
{
	for (auto field : fields_)
	{
...
std::shared_ptr<GameField> GameFieldManager::getFieldByName(const QString& name)
{
	for (auto field : fields_)
	{
		if (field->getName() == name)
...
std::list<QString> GameFieldManager::getNames()
{
